            ...
            if (item.name == 'APP_LAYOUT_WEBVIEW_ENABLED') {
                input.setValidator(value => {
                    if (!config.app_layout_webview && value) {
                        throw new Error('LAYOUT WEBVIEW No puede quedar vacio.')
                    }
                });
            }
...
const main = async () => {
    ...
    appConfigList.observe(async (event, config) => {
        if (event == 'update') {
            ...
            if (response.status === 200) {
                showToastSuccess('Configuracion guardada exitosamente!');
                return;
            }
    ...
    appConfigList.observe(async (event, config) => {
        if (event == 'delete') {
            ...
            if (response.status === 204) {
                showToastSuccess('Configuracion eliminada exitosamente!');
                main();
                return;
            }
    ...
    appConfigList.observe(async (event, id) => {
        if (event == 'toggle') {
            ...
            if (response.status === 200) {
                showToastSuccess('Configuracion actulizada exitosamente!');
                main();
                return;
            }
            ...
            showToastError('Error al actulizar configuración!');
        }
    });
    ...
    appConfigList.observe(async (event, config) => {
        if (event == 'import') {
            ...
            showToastError('Error al importar configuración!');
        }
    });
    ...
    appConfigList.observe(async (event, config) => {
        if (event === 'sync') {
            ...
            showToastSuccess('Felicidades, configuración sincronizada exitosamente!');
            showLoading();
            getConfigApp();
        }
    });
