...
const main = async () => {
    ...
    categoryList.register(new Observer('remove', async category => {
        ...
        try {
            ...
            if (response.status == 204) {
                showToastSuccess(`Categoria ${category.name} borrada exitosamente!`)
                return
            }
            ...
            showToastError(`Error al borrar categoria ${category.name}!`)
        }
    ...
    categoryList.register(new Observer('update', async category => {
        ...
        if (data.status == 200) {
            showToastSuccess(`Categoria ${category.name} atualizada exitosamente!`)
            return;
        }

        showToastError(`Errorr al actualizar categoria ${category.name}!`)
    }))
