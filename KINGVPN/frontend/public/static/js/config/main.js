...
const getConfigAndCategory = async () => {
    ...
    try {
        ...
        errorElement.render();
        showToastError('Error con el panel.');
    }
};
...
list.register(new Observer('append', async item => {
    ...
    try {
        ...
        if (response.status == 201) {
            ...
            showToastSuccess('Configuracion creada exitosamente!');
            renderApp();
            return;
        }
        ...
        showToastError('No fue posible crear su configuracion!');
        list.remove(item.id);
        categories.remove(item.id);
    }
...
list.register(new Observer('update', async item => {
    ...
    try {
        ...
        showToastError('No fue posibel actualizar la configuracion!');
    } finally {
        renderApp();
    }
}));
...
list.register(new Observer('remove', async id => {
    ...
    try {
        ...
        if (response.status == 204) {
            showToastSuccess('Configuracion borrada exitosamente!');
            return;
        }
        ...
        showToastError('No fue posible borrar la configuracion!');
    } finally {
        renderApp();
    }
}));
